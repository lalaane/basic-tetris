document.addEventListener('DOMContentLoaded', () => {
	...
	function moveLeft() {
		...
		//if it's not at the left edge, move one square left
		if (!isAtLeftEdge) currentPosition -= 1;
		//if there's already some other element to the left, move back, so it seems like it didn't move
	...
	function moveRight() {
		...
		//if it's not at the right edge, move one square right
		if (!isAtRightEdge) currentPosition += 1;
		// if there's blockage move back to same position
		if (current.some((index) => squares[currentPosition + index].classList.contains('taken'))) {
			currentPosition -= 1;
		}
		draw();
